...
func (self *RedisCache) Listen(ctx context.Context, key string) (string, error) {
	...
	const errMsg = "pubsub message from channel %q: %w"
	go func() {
		m, err := pubsub.ReceiveMessage(ctx)
		if ctx.Err() != nil {
			err = fmt.Errorf(errMsg, key, context.Cause(ctx))
		} else if err != nil {
			err = fmt.Errorf(errMsg, key, err)
		}
